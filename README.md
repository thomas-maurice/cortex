...
| `cortex backup` / `backup list\|download\|delete <name>` | **Admin.** Full server-side backup: every tenant's memories + summaries plus the user/API-key registry (hashes), written as a timestamped JSON file to the server's backup dir — and, when the offsite (S3) config is enabled, also uploaded to the bucket. Manage the files with `list`/`download`/`delete`. Set `BACKUP_INTERVAL` (+ `BACKUP_KEEP`, default 7) on the server for automatic periodic backups with retention; S3 credentials are configured by an admin in the web UI's Backup view (secret is write-only, works with AWS/MinIO/R2). |
| `cortex backup self [-o file]` | **Any user.** Download a backup of YOUR OWN data (memories + conversation summaries) to a local file. |
| `cortex restore <name> \| --file <local> [--yes]` | **Admin.** Restore a full backup — by server-side filename or by uploading a local copy: recreates missing users/API keys (existing ones untouched), re-queues every record into its original tenant; the worker re-embeds. Safe to re-run (upsert-by-id). |
