...
| `backup` / `backup list\|download\|delete <name>` | Admin-only FULL server backup: all tenants' memories + summaries + user/API-key registry (hashes) → timestamped `cortex-full-backup-*.json` in BackupDir, plus S3 upload when the offsite config is enabled (stored in the `CortexSettings` Weaviate class; secret write-only over RPC; minio-go, any S3-compatible endpoint). Periodic: `BACKUP_INTERVAL` + `BACKUP_KEEP` env on the server. Pruning only ever touches `cortex-full-backup-*` files, never reindex snapshots (`cortex-backup-*`). |
| `backup self [-o file]` | ANY user: download a backup of their own tenant (memories + summaries) locally. |
| `restore <name> \| --file <local> [--yes]` | Admin-only full restore (server-side name XOR uploaded local file): recreates missing users/keys (existing skipped), re-queues all records to their original tenants (worker re-embeds). Idempotent. |
